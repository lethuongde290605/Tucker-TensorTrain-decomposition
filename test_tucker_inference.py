"""
test_tucker_inference.py
----------------------------
Quick smoke-test for the Tucker-only-compressed OPT model (OPTTuckerDecoderLayer).

Usage
-----
# Minimal (CPU-only, OPT-125m):
python test_tucker_inference.py --model facebook/opt-125m

# With GPU and custom number of calibration samples:
python test_tucker_inference.py --model facebook/opt-125m --nsamples 4 --avg_dim 2
...
python test_tucker_inference.py --model facebook/opt-125m --no_baseline

What it does
------------
1. Loads the original OPT model (baseline).
2. Optionally runs greedy generation on the baseline for comparison.
3. Runs the Tucker-only decomposition pipeline (same as main_eigen_attn.py eigenattn()).
...
def main():
    parser = argparse.ArgumentParser(description="Tucker-only OPT inference smoke-test")
    ...
    parser.add_argument("--num_factors",     type=int,   default=5,
                        help="Number of Tucker factors per dimension.")
    ...
    print(f"\n{'='*60}")
    print(f"  Tucker OPT Inference Test")
    print(f"{'='*60}")
    ...
    print("\n[3/4] Running Tucker-only decomposition (this may take a while) ...")
    ...
    logger = logging.getLogger("tucker_test")
